...
def type_trans_fun(type_fun: Union[str, Any], value: Any):
    ...
    if isinstance(type_fun, str):
        if type_fun.lower() not in type_map:
            raise ValueError("type function: `{}` dose not supported".format(type_fun))
        else:
...
str2intlist = lambda x: str2list(x, trans_type=int)
str2floatlist = lambda x: str2list(x, trans_type=float)
str2intdict = lambda x: str2dict(x, trans_type=int)
str2floatdict = lambda x: str2dict(x, trans_type=float)


def str2json(input_str: str):
    try:
        data = eval(input_str)
...
def str2scale(input_str: str):
    ...
    if left_side == "[" and right_side == "]":
        ...
        raise ValueError("Unknown left_side={} or right_side={}".format(left_side, right_side))
...
def str2dayno(x: str, mode: str = "patten") -> Union[str, List[str]]:
    ...
    if "~" in x:
        ...
        raise Exception("Unknown input='{}'".format(x))
...
def str2loss(loss_name: str) -> List[str]:
    ...
    if package == "":
        raise ValueError("Expect loss str is 'package.class', got {}, please given full package route.".format(loss))
    else:
    ...
    try:
        ...
        raise AttributeError("""Load Loss Error\nSupport Loss :\n[\n    {}\n]\n Detail: {}""".format(info, str(e)))
    return loss